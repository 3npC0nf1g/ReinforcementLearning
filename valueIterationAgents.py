...
class ValueIterationAgent(ValueEstimationAgent):
  ...
  """
        ...
        for i in range(iterations):  # Pour une valeur dans l'ensemble d'itération on exécute la boucle
            ...
            for s in mdp.getStates():  # On parcours tous les états possibles du MDP
                ...
                if len(actions) > 0:  # On vérifie qu'au moins une action est disponible
                    ...
                    self.best_action_dict[s] = bestaction  # On stocke l'action associée à la meilleure valeur pour
                    # l'état s
                    nextvalues[s] = bestscore  # Mise à jour de la meilleure valeur trouvée dans la copie de valeur pour
                    # l'état actuel
            self.values = nextvalues  # Après avoir calculer la meilleure valeur trouvée pour tous les états on met à
            # jour les valeurs actuelles avec celles de la copie
    ...
    """
        return self.values[
            state]  # Retourne la meilleure valeure corresponde à l'état actuel, obtenue après l'itération des valeurs
    ...
    def getPolicy(self,
                  state):  # Retourne l'action optimale dans un état donné telle que déterminée pendant l'itération de valeurs
        """
